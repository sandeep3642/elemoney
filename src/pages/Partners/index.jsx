...
const SubAdmin = () => {
  ...
  const filteredData = Partner.filter(item =>
      ...
      {step === 1 && (
        <div className="">
          ...
          <button className='btn btn-primary ml-auto block'>
            <FaPlus className='inline-block mr-2' /> Add Partner
          </button>
          <div className="border border-[#ddd] rounded-lg my-4">
            <div className="flex justify-between items-center py-2 px-4">
              <h2 className="text-sm text-gray-800">partner list</h2>
            ...
            <table className="w-full border-collapse border-t border-[#ddd] text-sm">
              <thead>
                <tr className="bg-gray-100 text-left border-b border-[#ddd]">
                  <th className="p-3 font-semibold ">Partner Name</th>
                  <th className="p-3 font-semibold ">Mobile No.</th>
                  <th className="p-3 font-semibold ">Email</th>
                  <th className="p-3 font-semibold ">Number of Customers</th>
              ...
              <tbody>
                {filteredData.map((item, index) => (
                  <tr key={index} className="[&:not(:last-child)]:border-b border-[#ddd]">
                    <td className="p-3 cursor-pointer" onClick={() => setStep(2)}>{item.PartnerName}</td>
                    <td className="p-3 cursor-pointer" onClick={() => setStep(2)}>{item.Mobile}</td>
                    <td className="p-3">{item.Email}</td>
                    <td className="p-3">{item.Customersno}</td>
                    <td className="p-3">{item.Invested}</td>
                    <ActionsDropdown />
                  </tr>
                ))}
              </tbody>
      ...
      {step === 2 && (
        <div className="max-w-[1000px]">
          <div className="sub-admin-info flex flex-col gap-5">
            ...
            </button><h3> Partner Information</h3></div>
            <div>

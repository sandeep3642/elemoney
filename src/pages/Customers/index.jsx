...
const Customers = () => {
  ...
  const filteredData = customersList && Array.isArray(customersList) && customersList.filter(item =>
  ...
  return (
    <>
      {step === 1 && (
        <div className="">
          <button className='btn btn-primary ml-auto block'><FaPlus className='inline-block mr-2' /> Add Customer</button>
            ...
            <table className="w-full border-collapse border-[#ddd] text-sm">
              ...
              <tbody>
                {filteredData.map((item, index) => (
                  ...
                  >
                    <td className="p-3 cursor-pointer text-blue-700">{item.name || "NA"}</td>
                    <td className="p-3 cursor-pointer">{item.email || "NA"}</td>
                    <td className="p-3">{item.mobile || "NA"}</td> 
                    <td className="p-3">{getMessageName(item?.kyc_status) || "NA"}</td>
        ...
        <div className="">
          <div className="row">
            ...
            <div className="w-fit mb-4">
              <div className='border border-[#aeaeae] rounded-lg p-2 cursor-pointer hover:bg-black hover:text-white duration-300' onClick={() => setStep(1)}>
            ...
            <div className="col-12">
              <div class="flex items-center justify-between p-4 border border-[#b3b35c] rounded-lg bg-[#f9f9e7] w-full mb-4">
                <div>
                  <h1 class="text-[#8a8a2e] text-xl font-bold">KYC Verification</h1>
                  <p class="text-gray-500">
                    {
                      customerDetails?.
                        kyc_verification?.status === "pending" ? "  KYC Verification need to be verified" : "  KYC Verification done"
                    }
                ...
                </div>
                {
                  ...
                  <button class="bg-black text-white py-2 px-4 rounded-lg">Verify Now</button>
                }
            ...
            <div className="col-12">
              {
                customerDetails?.kyc_status === "NOT_STARTED" ? (
                  <div className="flex items-center justify-between p-4 border border-[#FB5959] rounded-lg bg-[#FFEDEC] w-full mb-4">
                    <div>
                      <h1 className="text-[#FB5959] text-xl font-bold">KYC Pending</h1>
                  ...
                  <div className="flex items-center justify-between p-4 border border-[#36B37E] rounded-lg bg-[#E6F4EC] w-full mb-4">
                    <div>
                      <h1 className="text-[#36B37E] text-xl font-bold">KYC Done</h1>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div className="p-6 border border-[#ddd] rounded-lg flex items-start bg-white">
                      <div className="min-w-[190px]">
                        <h3 className="text-gray-500 text-xl font-normal">Customer Detail <img className="w-6 h-6 inline" src="./images/question-mark-circle.svg" /></h3>
                        <img alt="Customer profile picture" className="mt-5 rounded-full mr-4" height="120" src="./images/user1.png" width="120" />
                      </div>
                      <div className="mt-10">
                        <h2 className="text-xl font-semibold">
                          ...
                          <img className="w-6 h-6 float-right ml-1" src="./images/verifed_success.svg" />
                        </h2>
                        <p className="text-gray-600 mt-1">
                          Customer ID : 394028037
                        </p>
                        <p className="text-gray-600 mt-1">
                          <img className="w-6 h-6 float-left mr-3" src="./images/mail_ico.svg" /> {
                            customerDetails?.email
                          }
                        </p>
                        <p className="text-gray-600 mt-1">
                          <img className="w-6 h-6 float-left mr-3" src="./images/call_ico.svg" />  +91 {
                    ...
                    <div className="p-6 border border-[#ddd] rounded-lg bg-white flex flex-col w-full justify-between">
                      <h3 className="text-xl font-normal text-gray-500 mb-2">
                        Your Portfolio Value <img className="w-6 h-6 inline" src="./images/mdi_information-outline.svg" />
                      </h3>
                  ...
                  <div>
                    <div>
                      ...
                      <div class="bg-gray-200 p-2 rounded-lg flex items-center space-x-4 w-fit">
                        {tabs.map((tab) => (
                          <div key={tab.id}
                            className={`p-2 rounded-lg cursor-pointer ${activeTab === tab.id
                              ? "shadow bg-white"
                              : ""
                              }`}
                            onClick={() => setActiveTab(tab.id)}>
                            <span key={tab.id}
                              className={`font-normal text-lg ${activeTab === tab.id
                                ? ""
                                : "text-gray-500"
                                }`}
                            >{tab.label}</span>
